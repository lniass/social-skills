...
Current public onboarding uses the bundled fixed-origin guest helper and stops after questionnaire completion. The planned server-returned Handled verification flow is documented in `guest-questionnaire-flow.md`. Do not ask a public onboarding user to configure or authenticate MCP. Do not use MCP for login, subscription, consent, guest-draft claim, or as a fallback when Handled verification is unavailable.
